...
with open("singaporedt.pkl","rb") as files:
    model=pickle.load(files)
with open('label_encoders1.pkl', 'rb') as f:
    label_encoders1 = pickle.load(f)
...
if button1:
    ...
    for column in new_data_df.select_dtypes(include=['object']).columns:
            if column in label_encoders1:
                le = label_encoders1[column]
                # Handle unseen labels
                new_data_df[column] = new_data_df[column].apply(lambda x: le.transform([x])[0] if x in le.classes_ else -1)
    new_data_scaled = loaded_pipeline1.transform(new_data_df)
